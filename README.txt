...
For the second release of Romanian UD trees, the team of the SSPR project (Verginica Mititelu, Elena Irimia, Cenel-Augusto Perez, Radu Ion, Radu Simionescu and Cătălina Mărănduc) continued the human-supervised, automatic mapping of the two original treebanks (RACAI-RoTb and UAIC-RoTb) to the UD annotation style by first doing an automatic mapping from the original label set to the UD label set, followed by a manual (word by word) inspection of the machine-assigned UD labels.
